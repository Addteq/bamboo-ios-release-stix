...
//  Bamboo
//
//  Created by Weifeng Zheng on 7/10/13.
...
typedef void (^myAFNetworkReachabilityStatusBlock)(myAFNetworkReachabilityStatus status);
#else
...
extern NSArray * myAFQueryStringPairsFromDictionary(NSDictionary *dictionary);
extern NSArray * myAFQueryStringPairsFromKeyAndValue(NSString *key, id value);

NSString * myAFQueryStringFromParametersWithEncoding(NSDictionary *parameters, NSStringEncoding stringEncoding) {
...
@property (readwrite, nonatomic, copy) myAFNetworkReachabilityStatusBlock networkReachabilityStatusBlock;
#endif
...
- (id)initWithBaseURL:(NSURL *)url {
    ...
    self.parameterEncoding = myAFFormURLParameterEncoding;
...
static void AFNetworkReachabilityCallback(SCNetworkReachabilityRef __unused target, SCNetworkReachabilityFlags flags, void *info) {
    ...
    myAFNetworkReachabilityStatusBlock block = (__bridge myAFNetworkReachabilityStatusBlock)info;
    if (block) {
        block(status);
    }
    
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:myAFNetworkingReachabilityDidChangeNotification object:nil userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithInteger:status] forKey:myAFNetworkingReachabilityNotificationStatusItem]];
    });
}

static const void * AFNetworkReachabilityRetainCallback(const void *info) {
    return (__bridge_retained const void *)([(__bridge myAFNetworkReachabilityStatusBlock)info copy]);
}
...
- (void)startMonitoringNetworkReachability {
    ...
    myAFNetworkReachabilityStatusBlock callback = ^(myAFNetworkReachabilityStatus status) {
                                ...
                                parameters:(NSDictionary *)parameters
{
    ...
    if (parameters) {
        if ([method isEqualToString:@"GET"] || [method isEqualToString:@"HEAD"] || [method isEqualToString:@"DELETE"]) {
            url = [NSURL URLWithString:[[url absoluteString] stringByAppendingFormat:[path rangeOfString:@"?"].location == NSNotFound ? @"?%@" : @"&%@", myAFQueryStringFromParametersWithEncoding(parameters, self.stringEncoding)]];
            [request setURL:url];
        } else {
            ...
            switch (self.parameterEncoding) {
                case myAFFormURLParameterEncoding:;
                    [request setValue:[NSString stringWithFormat:@"application/x-www-form-urlencoded; charset=%@", charset] forHTTPHeaderField:@"Content-Type"];
                    [request setHTTPBody:[myAFQueryStringFromParametersWithEncoding(parameters, self.stringEncoding) dataUsingEncoding:self.stringEncoding]];
                    break;
                case myAFJSONParameterEncoding:;
                    [request setValue:[NSString stringWithFormat:@"application/json; charset=%@", charset] forHTTPHeaderField:@"Content-Type"];
                    [request setHTTPBody:[NSJSONSerialization dataWithJSONObject:parameters options:0 error:&error]];
                    break;
                case myAFPropertyListParameterEncoding:;
                              ...
                              constructingBodyWithBlock:(void (^)(id <myAFMultipartFormData> formData))block
{
    ...
    __block myAFStreamingMultipartFormData *formData = [[myAFStreamingMultipartFormData alloc] initWithURLRequest:request stringEncoding:self.stringEncoding];
...
- (id)copyWithZone:(NSZone *)zone {
    myAFHTTPClient *HTTPClient = [[[self class] allocWithZone:zone] initWithBaseURL:self.baseURL];
...
@interface myAFMultipartBodyStream ()
...
@property (nonatomic, strong) myAFHTTPBodyPart *currentHTTPBodyPart;
@end
