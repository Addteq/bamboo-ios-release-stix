...
- (void) viewWillAppear:(BOOL)animated {
    ...
    //Solve for BIP 286 Change teh word suggestion
    self.descriptionField.autocorrectionType= UITextAutocorrectionTypeNo;
...
- (IBAction)sendFeedback
{
    //Change here
    NSLog(@"dfdsfsdfd");
    //Check if the content is empty. If yes, pop up a toast message.
    if ([self.descriptionField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length <= 0) {
        ...
        [[[[[[iToast makeText:@"Please type the comment"]
    ...
    if (self.replyToIssue) {
        ...
        u_int length = 80;
        u_int toIndex = [description length] > length ? length : (int)[description length];
...
- (void)layoutActionButton:(UIButton *)button {
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        button.frame = CGRectMake(self.descriptionField.superview.frame.size.width - kJMCButtonSpacing,
                                  kJMCButtonSpacing + _buttonOffset + 20,
                                  44.0,
                                  44.0);
    }
