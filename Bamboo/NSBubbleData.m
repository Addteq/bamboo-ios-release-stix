...
const UIEdgeInsets textInsetsMine = {5, 12, 5, 21};
const UIEdgeInsets textInsetsSomeone = {5, 17, 5, 14};
//const UIEdgeInsets textInsetsMine = {5, 10, 11, 17};
//const UIEdgeInsets textInsetsSomeone = {5, 15, 11, 10};

- (id)initWithText:(NSString *)text date:(NSDate *)date type:(NSBubbleType)type fontColor:(UIColor *)color{
 ...
 //   CGSize size = [(text ? text : @"") sizeWithFont:font constrainedToSize:CGSizeMake(220, 9999) lineBreakMode:NSLineBreakByWordWrapping];
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0,0,correctWidth,correctHeight)];
...
+ (id)dataWithText:(NSString *)text date:(NSDate *)date type:(NSBubbleType)type fontColor:(UIColor *)color{
#if !__has_feature(objc_arc)
    return [[[NSBubbleData alloc] initWithText:text date:date type:type fontColor:color] autorelease];
#else
    return [[NSBubbleData alloc] initWithText:text date:date type:type fontColor:color];
#endif  
}
