...
//	Edited by Yung Chang on 3/4/13
        ...
        [client getPath:projectURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if (isResponseDict == 1) {
                ...
                if (self.startIndexForServerRequestForProjects == 0) {
                    ...
                    dispatch_queue_t requestForProjectThreadAgain = dispatch_queue_create("requestForProjectThreadAgain",NULL);
                    dispatch_async(requestForProjectThreadAgain, ^(void){
                    [self serverRequestForProjects];
                    });
                }
    ...
    dispatch_async(planRequest, ^(void){
        ...
        [client getPath:successURL parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if (isResponseDictForPlans == 1) {
                ...
                dispatch_queue_t requestForPlansThreadAgain = dispatch_queue_create("requestForPlansThreadAgain",NULL);
                dispatch_async(requestForPlansThreadAgain, ^(void){
                    [self serverRequestForPlans];
                });
            }
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
...
//// filter the project from the allprojectsArrayToStoreInPlist to avoid repeatation
       ...
       self.allprojectsArrayToStoreInPlist = [[arrayFromPlist arrayByAddingObjectsFromArray:nonRepeatingArray]mutableCopy];
        [self.allprojectsArrayToStoreInPlist writeToFile:plistPath atomically:YES];
    }
...
- (void)storePlansInPlist {
    ...
    NSArray *arrayFromPlist = [[NSArray alloc] initWithContentsOfFile:plistPath];
    if (self.startIndexForServerRequestForPlans == 0) {
        [self.allPlansArrayToStoreInPlist writeToFile:plistPath atomically:YES];
    } else {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    {
        NSLog(@"ipad");
        UIView *customView = [[UIView alloc] initWithFrame:CGRectMake(0,0,400,270)];
        Project *project = [[Project alloc]init];
        ...
        /////////((((
        headerLabel.frame = CGRectMake(0,-10,740,50);
        //  headerLabel.
        NSString *projectNameForCellText = [NSString stringWithFormat:@"  %@",projectName];
        headerLabel.text =  projectNameForCellText;
        ...
        if ([project.plans count]==0 ) {
            ...
            UIButton *button = [[UIButton alloc]initWithFrame:CGRectMake(self.view.bounds.size.width -100,15,60,20)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   //         cell.planStatus.frame = [CGRectMake(<#CGFloat x#>, /, //, /)]
            cell.planStatus.image = image;
        } else {
            UIImage *image = [UIImage imageNamed: @"redX.png"];
            if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
            {
                image = [UIImage imageNamed: @"redX.png"];
            }
            cell.planStatus.image = image;
        }
        return cell;
